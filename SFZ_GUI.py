...
class IDCardManager:
    ...
    def validate_id_card(self, id_card):
        ...
        # 如果是18位，验证校验码
        if len(id_card) == 18:
            if not self._validate_check_code(id_card):
                return False, "身份证号码校验码错误"
        
        return True, "验证通过"
    
    def _validate_check_code(self, id_card):
        """
        验证18位身份证号码的校验码
        :param id_card: 18位身份证号码
        :return: 验证结果
        """
        # 校验码计算规则
    ...
    def get_area_info(self, id_card):
        ...
        # 获取前六位地区代码对应的完整地区信息
        area_code = id_card[:6]
        full_area = self.AREA_CODES.get(area_code, f"未知地区(代码:{area_code})")
    ...
    def parse_id_card(self, id_card):
        ...
        # 获取各项信息
        area_info = self.get_area_info(id_card)
        birthday_info = self.get_birthday(id_card)
...
class IDCardGUI:
    ...
    def __init__(self):
        self.manager = IDCardManager()
        self.root = tk.Tk()
        self.root.title("身份证号码信息解析器")
    ...
    def create_widgets(self):
        ...
        self.id_card_entry = ttk.Entry(input_frame, 
                                     textvariable=self.id_card_var, 
                                     width=35,
                                     font=("微软雅黑", 12))
        self.id_card_entry.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=(0, 10))
        
        # 绑定回车键
        self.id_card_entry.bind('<Return>', lambda event: self.parse_id_card())
        
        # 按钮区域
        button_frame = ttk.Frame(main_frame)
        button_frame.grid(row=2, column=0, columnspan=2, pady=(0, 20))
        
        parse_button = ttk.Button(button_frame, 
                                text="🔍 解析身份证", 
                                command=self.parse_id_card,
    ...
    def format_output(self, result):
        ...
        output = []
        output.append("=" * 50)
        output.append("身份证号码信息解析结果")
        output.append("=" * 50)
        output.append(f"身份证号：{result['id_card']}")
        output.append(f"号码长度：{result['length']}位")
        output.append(f"所在地区：{result['area']}")
        
        birthday = result['birthday']
        output.append(f"出生日期：{birthday['date']}")
        output.append(f"出生年份：{birthday['year']}年")
        output.append(f"出生月份：{birthday['month']}月")
        output.append(f"出生日期：{birthday['day']}日")
        output.append(f"年龄：{birthday['age']}岁")
        
        output.append(f"性别：{result['gender']}")
        output.append("=" * 50)
        
        return '\n'.join(output)
    
    def parse_id_card(self):
        """解析身份证号码并显示结果"""
        id_card = self.id_card_var.get().strip()
        
        if not id_card:
            messagebox.showwarning("输入错误", "请输入身份证号码！")
            return
        
        result = self.manager.parse_id_card(id_card)
